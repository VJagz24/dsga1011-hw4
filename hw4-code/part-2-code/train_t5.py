...
def get_args():
    ...
    parser.add_argument('--scheduler_type', type=str, default="linear", choices=["none", "cosine", "linear"], help="Whether to use a LR scheduler and what type to use if so")
    ...
    parser.add_argument('--max_n_epochs', type=int, default=15, help="How many epochs to train the model for")
    parser.add_argument('--patience_epochs', type=int, default=5, help="If validation performance stops improving, how many epochs should we wait before stopping?")
    ...
    parser.add_argument('--max_generation_length', type=int, default=512, help="Maximum length for generated SQL queries")
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for batch in tqdm(dev_loader, desc="Evaluating"):
            ...
            outputs = model.generate(input_ids=encoder_input, attention_mask=encoder_mask, max_length=args.max_generation_length, num_beams=args.num_beams, early_stopping=True)
            for output in outputs:
                generated_query = tokenizer.decode(output, skip_special_tokens=True)
    ...
    error_rate = num_errors / len(error_msgs) if len(error_msgs) > 0 else 0.0
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    save_queries_and_records(generated_queries, model_sql_path, model_record_path)
    print(f"Generated {len(generated_queries)} SQL queries for test set")
